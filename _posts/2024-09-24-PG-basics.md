...
<br>
#### **Implications of Converting the Gradient update from $\nabla_\theta \mathbb{E} [R(\tau)] \rightarrow \mathbb{E} [\nabla_\theta R(\tau)]$**

Although this update is unbiased in expectation, it has very high variance due to the sparse reward signals and noise in trajectories, and does not work well in practice. Suppose we want to evaluate the expected gradient $\nabla_\theta J(\theta)=\mathbb{E}[\nabla_\theta \log \pi_\theta \cdot R(\tau)]$ (short-hand), we can estimate $\nabla_\theta J(\theta)$ by generating $N$ independent rollouts and taking the average of the gradients for each of the $n$ trajectories $\tau$:

$$
\tilde \nabla_\theta J(\theta) = \frac{1}{N} \sum_{n=1}^N \nabla_{\theta} \log \pi_\theta (\tau_n) \cdot R_n(\tau_n)
$$

This is a Monte-Carlo estimator, i.e. unbiased estimator of $\nabla_\theta J(\theta)$. The error due to variance of the gradient estimator is 

$$
Var(\tilde \nabla_\theta J(\theta)) = \mathbb{E}[(\tilde \nabla_\theta J(\theta) - \mathbb{E}[\tilde \nabla_\theta J(\theta)])^2] \\
$$
...
Subtracting a **control variate** (more colloquially known in RL world as a baseline term $b$) can reduce the variance of the gradient estimate. If the control does not depend on $\theta$, the gradient expectation doesn't change and continues to be unbiased, since $\mathbb{E}[\nabla_\theta b)] = 0$, $\mathbb{E}[\nabla_\theta(R-b)]=\mathbb{E}[\nabla_\theta R]$.
