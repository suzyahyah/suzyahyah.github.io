...
<u>Comments</u> Looking at attention weights for interpretability are just problematic. With just a tiny bit of work (this is probably 2 lines of code) we can get something obviously better. There's really no point in defending them any longer.
...
**Why does repetition happen in NLG?** Repetition causes token level negative log likelihood to decrease (more and more probable sequences), and this is even worse for Transformers. I think the best explanation of this is that repetition is not weird for LMs, but repetition is weird for us humans. 
